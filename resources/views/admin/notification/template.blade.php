        ...
        <div class="col">
            ...
            <div class="row mt-3">
                ...
                <div class="col-2">
                    ...
                    <h4 class="card-title text-center mt-5">{{__('Available image src')}}</h4>
                    <div class="text-center">
                        @php
                            $loop = array(
                                'Black Logo' => asset('storage/images/app/'.App\AdminSetting::first()->black_logo),
                                'White Logo' => asset('storage/images/app/'.App\AdminSetting::first()->white_logo),
                                'Small Icon' => asset('storage/images/app/'.App\AdminSetting::first()->favicon)
                            );
                        @endphp
                        @foreach ($loop as $key => $item)
                            <button class="btn-sm btn btn-primary mt-2 rtl-float-none"  id="TextToCopy" onclick="copy_url('{{$item}}')"  data-toggle="tooltip" data-original-title="{{__('Click to Copy')}}"> {{$key}} </button><br>
                        @endforeach
                    </div>
                </div>
            </div>
